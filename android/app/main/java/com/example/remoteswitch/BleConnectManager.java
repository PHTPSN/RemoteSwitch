...
public class BleConnectManager {
    ...
    public void connect() {
        if (device == null) {
            listener.onConnectFailed(context.getString(R.string.connect_failed));
            return;
        }
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
            listener.onConnectFailed(context.getString(R.string.bluetooth_permission_failed));
            return;
        }
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (ActivityCompat.checkSelfPermission(context, Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
                listener.onConnectFailed(context.getString(R.string.permissions_missing));
                return;
            }
