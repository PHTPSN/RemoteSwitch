...
You should install the lower servo in the right position and the upper servo upsidedown.
*/
...
const long SLEEP_DURATION = 11 * 3600; // 12 hours
...
private:
  ...
  public:
    ...
    void onWrite(BLECharacteristic *pChar) {
      ...
      if(value == "1") {
        ...
        activateServo(SERVO_B_PIN, servoB, 30); // Turn on, the upper servo rotate 30°
      } 
      else if(value == "0") {
        Serial.println("Received '0'");
        activateServo(SERVO_A_PIN, servoA, 30);  // Turn off, the lower servo rotate 30° 
      }
    }
  ...
  private:
    ...
    void activateServo(int servoPin, Servo &servo, int angle) {
      if(!isActivated){
        isActivated = true;
...
void loop() {
  ...
  if(connectStartTime > 0 && millis() - connectStartTime >= 3 * 60 * 1000 && MyBLEController->pServer) {
